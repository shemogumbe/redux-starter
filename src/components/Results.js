...
class Results extends Component{
    constructor(props){
        ...
        console.log(this.store.getState().angular)
    }
    ...
    votesVuejsInPercent(){
        if(this.store.getState().vuejs){
            return (this.store.getState().vuejs / (this.store.getState().vuejs + this.store.getState().angular  + this.store.getState().angular)) * 100;
        } else{
            return 0;
        }
    }
    ...
    render(){
        return(
            <div>
                ...
                <span className="label label-success"> Vuejes: {this.votesVuejsInPercent().toFixed(2) + "%"}</span>
                <div className="progress progress-striped active">
